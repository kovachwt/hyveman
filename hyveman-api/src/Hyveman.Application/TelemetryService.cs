        ...
        foreach (var f in facts)
        {
            ...
            if (stored && host is not null)
            {
                var vms = f.Vms.Select(v => new VmRecord(
                    ...
                    Stale: f.Stale, CollectedAt: f.CollectedAt,
                    ReplicationState: v.ReplicationState, ReplicationHealth: v.ReplicationHealth,
                    ReplicationLastApplyTime: v.ReplicationLastApplyTime)).ToList();
                // VM heartbeat transitions (DESIGN §4.4 rule type 5) and VM
                // replication crossings (rule type 7) are evaluated BEFORE the
                // latest-wins upsert so the evaluator can read the previous
                // facts from the store (D3). Stale facts are re-emitted old
                // data after a WMI timeout (PROTOCOL §7.4) — not a state
                // change — so they are stored (the UI marks them stale) but
                // never evaluated. Derived alerting must never fail an
                ...
                if (!f.Stale)
                {
                    try
                    {
                        ...
                        await evaluator.OnVmReplicationChangedAsync(host.Id, vms, receivedAt, ct);
                    }
                    catch (Exception ex)
                    {
                        log.LogError(ex, "VM alert evaluation failed for source {sourceId}; facts still stored", sourceId);
                    }
                }
